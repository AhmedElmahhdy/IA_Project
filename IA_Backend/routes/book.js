...
// 2- update
router.put("/update/:id",admin,  upload.single("image"),
body("Title").isString().withMessage("please enter a valid book title").isLength({min:5})
.withMessage("book title should be at least 5 characters"),
...
// user => list , view 
// filter => filter order by ranknumber 
router.get("/filter",user,authorize,async(req,res) =>
{
    const query = util.promisify(conn.query).bind(conn);
    //const RankNumber = req.bodys.RankNumber;
    //let search = "";
    const books= await query(`select * from book ORDER BY RankNumber`);
    books.map(book =>{
...
// filter_ISBN  => filter order by ISBN
router.get("/filter_ISBN",user,authorize,async(req,res) =>
{
    const query = util.promisify(conn.query).bind(conn);
    //const RankNumber = req.bodys.RankNumber;
    //let search = "";
    const books= await query(`select * from book ORDER BY ISBN`);
    books.map(book =>{
        book.Photo_URL = "http://" + req.hostname + ":4000/" + book.Photo_URL
    })
     res.status(200).json(books)
...
router.post("/send_borrow/:id/:user_id",
 //  authorized,
  async (req, res) => {
    try {
      ...
      const checkBorrowExists = await query("SELECT * FROM Borrow_Books WHERE book_id = ? AND user_id = ?", [req.params.id, req.params.user_id]);
      ...
      const countBorrowRequests = await query("SELECT COUNT(*) AS count FROM Borrow_Books WHERE user_id = ?", [req.params.user_id]);
      ...
      await query("INSERT INTO Borrow_Books SET ?", [borrow]);
